        ...
        {wishlistItems.length === 0 ? (
          ...
          <div className="">
            {wishlistItems.map((item) => (
              <div
                key={item._id}
                className="border w-1/2 flex justify-center items-center rounded-lg p-4 shadow-md hover:shadow-lg transition duration-300"
              >
                <img
                  src={item.image}
                  alt={item.name}
                  className="h-48 w-full max-w-[100px] rounded mb-4 cursor-pointer"
