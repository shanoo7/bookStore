...
useEffect(() => {
  ...
  return (
    <>
      <div className='max-w-screen-4xl py-5 mx-auto'>
        ...
        {
  searchBook.length >= 1 ? (
    <div className='py-2 light mx-5 '>
      <Slider
        {...settings}
        slidesToShow={Math.min(3, searchBook.length)}
        slidesToScroll={Math.min(3, searchBook.length)}
        infinite={searchBook.length > 3}
      >
        {searchBook.map((item) => (
          <div key={item._id}>
            <Cards item={item} />
    ...
    <div className="text-center text-gray-500 py-10">No matching courses found.</div>
  ) : (
