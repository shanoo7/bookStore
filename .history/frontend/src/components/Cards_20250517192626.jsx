...
function Cards({ item }) {
  ...
  const { handleSelectedBook,addToCart,addToWishlist} = useAuth();
...
<button onClick={()=>addToWishlist(item)}><BsHearts/></button>
              <button onClick={()=>addToCart(item)}><BsCartFill/></button>
              
              <button onClick={() =>{addToCart(item), navigate("/cart-page")}} className="badge border-none text-sm badge-outline bg-pink-500  py-4 px-2 cursor-pointer hover:bg-pink-700 duration:200">Buy Now</button>
            </div>
          </div>
        </div>
      </div>
