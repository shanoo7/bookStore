...
function Cards({ item }) {
  ...
  const { handleSelectedBook,addToCart,toggleWishlist,liked,isLiked} = useAuth();
...
<button onClick={()=>{toggleWishlist(item), isLiked(!liked(item))}}><GoHeartFill className={`${liked ? " text-red-600" : ""}`}/></button>
