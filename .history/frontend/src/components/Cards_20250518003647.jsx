...
function Cards({ item }) {
  ...
  const isLiked = wishlistItems.some((wishItem) => wishItem._id === item._id);
...
<button onClick={()=>{addToWishlist(item), toggleWishlist(item)}}><GoHeartFill className={`${isLiked ? "text-red-500":"text-white"}`}/></button>
