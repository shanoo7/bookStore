...
//   const { handleSelectedBook,addToCart,addToWishlist,wishlistItems,toggleWishlist} = useAuth();
...
// <button onClick={()=>toggleWishlist(item)}><GoHeartFill className={`${isLiked(item._id) ? "text-red-500":"text-white"}`}/></button>
...
function Cards({ item }) {
  const { handleSelectedBook, addToCart, toggleWishlist, wishlistItems } = useAuth();
  const isLiked = (id) => wishlistItems.some((wishItem) => wishItem._id === id);
  const navigate = useNavigate();
        ...
        <div className="card-body">
          ...
          <div className="card-actions justify-evenly items-center text-2xl">
            ...
            <button onClick={() => toggleWishlist(item)}>
              <GoHeartFill className={`${isLiked(item._id) ? "text-red-500" : "text-white"}`} />
            </button>
            <button onClick={() => addToCart(item)}>
              <BsCartFill />
            </button>
            <button
              onClick={() => { addToCart(item); navigate("/cart-page"); }}
              className="badge border-none text-xs badge-outline bg-pink-500 py-4 px-2 cursor-pointer hover:bg-pink-700 duration:200"
            >
              Buy Now
            </button>
          </div>
