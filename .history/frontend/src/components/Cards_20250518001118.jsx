...
function Cards({ item }) {
  ...
  const { handleSelectedBook,addToCart,addToWishlist,like,setLike} = useAuth();
...
<button onClick={()=>{addToWishlist(item), setLike(!like)}}><GoHeartFill/></button>
              <button onClick={()=>addToCart(item)}><BsCartFill className=' text-red-500'/></button>
